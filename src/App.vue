<template>
 <div id="app">
    <div v-if="!navigatorIsOnline() || fetchNetworkError" style="position:absolute; width: 100%;height:24px;color:white; padding-top:5px; font-size: min(4vw,20px);height: min(6vw,30px);text-align:center; background-color:grey; opacity:0.8">You are currently OFFLINE</div>
    <main>
      <div id="loaderId" v-if="showLoader" class="loader">
        <div id="loaderLottieId" class="loader-lottie">
          <lottie :style="'max-width:100%;height:auto;'" :options="defaultOptions" :height=lottieSize :width=lottieSize v-on:animCreated="handleAnimation"/>
        </div>
      </div>
      <div v-show="false" style="font-size:24px; color:white;position:absolute; top:0;left:0">V28</div>
      ...
      <div class="search-box">
        ...
        />
        <i id="searchIcon" @click="searchFocus && fetchWeather(query)" 
          class="fas fa-search-location" style="font-size:28px;color:white; margin-left:-45px;z-index:99; text-shadow: 3px 2px rgba(0, 0, 0, 0.5);" 
          aria-hidden="true" v-bind:class="{ transparentItem: !searchFocus, opacItem: searchFocus }"></i>
        <i @click="setGPSdata()" class="fas fa-street-view" style="font-size:36px;color:white; margin-left: min(50px,10%); text-shadow: 2px 3px rgba(0, 0, 0, 0.75);" aria-hidden="true"></i>
      </div>
      <div class="weather-wrap" v-if="currentCityDetail!==undefined && currentCityDetail.loc!==undefined">
        <div class="location-box">
          <div class="location">{{ currentCityDetail.loc }}</div>
          <div class="date">{{ dateBuilder(currentCityDetail.recordDate) }}</div>
        </div>

        <div class="weather-box">
          <img :src="getIconUrl(currentCityDetail.icon)" style="position:absolute;margin: -18px 0px 0px -32px; width: 40%; max-width: 100px; filter: drop-shadow(2px 2px 2px #222);">
          <div class="temp">{{ Math.round(currentCityDetail.deg) }}°c</div>
          <div class="weather">{{ currentCityDetail.prep }}</div>
        </div>
          <HistoryList :historyList="reverseTempList()" :key="updateKey">HistoryList Mus have been here :(</HistoryList>
      </div>
        <div v-show="false" id="stateDetail" style="color:red; position:absolute; bottom:0px; left:0px;" v-html="stateDetail"></div>
    </main>
  </div>
</template>
<script>
...
export default {
  ...
  async mounted() {
    ...
    document.getElementById('app').style.backgroundImage = "url('img/warm-day1.jpg')";
    window.addEventListener('online', this.handleStateChange);
    window.addEventListener('offline', this.handleStateChange);
    if(this.historyList!=null && this.historyList.list!==undefined && this.historyList.list.length>0){
  ...
  methods: {
    ...
    updateFormatDateToNow(){
      if(this.tempList.length>0){
        this.updateKey = this.updateKey>=Number.MAX_SAFE_INTEGER-1 ? 0 : this.updateKey+1;
        this.tempList.forEach((item)=>{item.toNow = GeneralUtils.formattedTimeToNOW(item.recordDate)})
      }
    },
    reverseTempList(){
    ...
    handleStateChange () {
      ...
      this.stateDetail = '' + timeBadge + ' Connection state changed to <b>' + state + '</b>.';
    ...
    fetchWeather(query){
      if(query=='') return;
        api.getWeatherByCityName(query).then(
        (response)=>{
...
*/
    ...
    fetchByLocation(lat,lon){
      ...
      api.getWeatherByCoordinates(lat,lon).then(
        (response)=>{
          console.log("axios-response:",response); 
          this.setResults(response.data);
        },
    ...
    setResults (results) {
      ...
      this.tempList.push({"loc":`${results.name.replace(' Province','')}, ${results.sys.country}`, "recordDate":new Date(),"prep":results.weather[0].description, "deg":Math.round(results.main.temp),"icon":icon});
      if(this.tempList.length>5) // Max Display List Size
        this.tempList.shift();
